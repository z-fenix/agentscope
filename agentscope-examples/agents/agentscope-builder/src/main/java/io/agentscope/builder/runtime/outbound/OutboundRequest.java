 ...
 * markdown). The {@link OutboundService} translates this into a
 * {@link io.agentscope.harness.agent.gateway.channel.OutboundAddress} and delegates to the matching
 * channel.
 *
 * @param channelId target channel id (must match a registered
 *                  {@link io.agentscope.harness.agent.gateway.channel.Channel}); required
 ...
 *                  channel's routing for {@code peerId} resolves to the same agent and refuses
 *                  delivery otherwise. Use this to prevent one agent from posting into a
 *                  channel/peer that is bound to a different agent. Skipped when {@code null} or
 *                  blank.
 */
