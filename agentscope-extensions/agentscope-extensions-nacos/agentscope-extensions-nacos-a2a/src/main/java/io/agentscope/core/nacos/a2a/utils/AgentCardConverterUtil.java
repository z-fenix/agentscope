...
/**
 * AgentCard Converter between Nacos {@link AgentCard} and A2A specification {@link AgentCard}.
 *
 * <p>This utility also converts sub-specifications when converting AgentCard:
 * <ul>
 *     <li>Nacos {@link AgentInterface} and A2A {@link AgentInterface}.</li>
 *     <li>Nacos {@link AgentProvider} and A2A {@link AgentProvider}.</li>
 *     <li>Nacos {@link AgentCapabilities} and A2A {@link AgentCapabilities}.</li>
 *     <li>Nacos {@link AgentSkill} and A2A {@link AgentSkill}.</li>
 * </ul>
 *
 * <p>Unlike the sub-specifications listed above, {@link SecurityScheme} is handled differently.
 * All types of A2A {@link SecurityScheme} are serialized and stored as a generic {@link Map}
 * representation, and Nacos does not retain or expose the concrete subtype of {@link SecurityScheme}.
 */
public class AgentCardConverterUtil {
  ...
  public static org.a2aproject.sdk.spec.AgentCard convertToA2aAgentCard(AgentCard agentCard) {
    ...
    return org.a2aproject.sdk.spec.AgentCard.builder()
        .version(agentCard.getProtocolVersion())
        .name(agentCard.getName()).description(agentCard.getDescription())
        .version(agentCard.getVersion()).iconUrl(agentCard.getIconUrl())
  ...
  private static List<Legacy_0_3_AgentInterface> convertToA2aAgentInterfaces(
      List<AgentInterface> nacosInterfaces) {
    if (nacosInterfaces == null || nacosInterfaces.isEmpty()) {
      return List.of();
    }
    return nacosInterfaces.stream().filter(Objects::nonNull)
        .map(AgentCardConverterUtil::convertToA2aAgentInterface).collect(Collectors.toList());
  }

  private static Legacy_0_3_AgentInterface convertToA2aAgentInterface(
      AgentInterface agentInterface) {
    return new Legacy_0_3_AgentInterface(agentInterface.getTransport(), agentInterface.getUrl());
  }
  ...
  private static List<org.a2aproject.sdk.spec.AgentSkill> convertToA2aAgentSkills(
    ...
    return nacosSkills.stream().map(AgentCardConverterUtil::convertToA2aAgentSkill)
        .collect(Collectors.toList());
  }

  private static org.a2aproject.sdk.spec.AgentSkill convertToA2aAgentSkill(AgentSkill nacosSkill) {
    return org.a2aproject.sdk.spec.AgentSkill.builder().id(nacosSkill.getId())
        .tags(nacosSkill.getTags()).examples(nacosSkill.getExamples()).name(nacosSkill.getName())
        .description(nacosSkill.getDescription()).inputModes(nacosSkill.getInputModes())
        .outputModes(nacosSkill.getOutputModes()).build();
  }
  ...
  public static AgentCard convertToNacosAgentCard(org.a2aproject.sdk.spec.AgentCard agentCard) {
    ...
    card.setProtocolVersion(agentCard.version());
    ...
    card.setSkills(
        agentCard.skills().stream().map(AgentCardConverterUtil::convertToNacosAgentSkill).toList());
  ...
  private static AgentInterface convertToNacosAgentInterface(
      Legacy_0_3_AgentInterface agentInterface) {
