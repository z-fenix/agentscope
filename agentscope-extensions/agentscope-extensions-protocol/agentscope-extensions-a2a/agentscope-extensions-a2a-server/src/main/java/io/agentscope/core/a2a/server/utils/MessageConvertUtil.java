...
public class MessageConvertUtil {
   ...
   * conversion from user-provided message lists must keep using
   * {@link #convertFromMsgToMessage(List, String, String)} directly so semantic message and block
   ...
   *
   * <p>Convert rule is revert from List of {@link Msg} to A2A {@link Message} in A2aAgent, step
   * with following:
   *
   * <ol>
  ...
  private static String getMsgId(Part<?> part) {
    Map<String, Object> metadata = getMetadata(part);
    if (null == metadata
        || null == metadata.get(MessageConstants.MSG_ID_METADATA_KEY)) {
  ...
  private static MsgRole getMsgRole(Part<?> part) {
    ...
    Object role = metadata.get(MessageConstants.MSG_ROLE_METADATA_KEY);
    if (role == null) {
      return null;
    }
    try {
