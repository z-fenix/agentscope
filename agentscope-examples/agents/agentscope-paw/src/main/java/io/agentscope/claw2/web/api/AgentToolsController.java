...
public class AgentToolsController {
   ...
   * {@code HarnessAgent.Builder.build()}. Used both for source attribution on {@code /active} and
   * as the {@code /catalog/builtins} response.
   */
  ...
  private ActiveToolsResponse configOnlyView(Path workspace, List<String> warnings) {
    ...
    Set<String> allow =
        cfg != null && cfg.getAllow() != null && !cfg.getAllow().isEmpty()
            ...
            : null;
    for (BuiltinToolInfo b : BUILTIN_TOOLS) {
      if (deny.contains(b.id())) {
        continue;
      }
      if (allow != null && !allow.contains(b.id())) {
        continue;
      }
